...
rawData$PSUs <- read_tsv("data/England/UKDA-5340-tab/tab/psu_eul_2002-2020.tab", col_names = TRUE,
                         cols_only(PSUID = "i", PSUStatsReg_B01ID = "i", PSUCountry_B01ID = "i")) 

# Read in households
rawData$households <- read_tsv("data/England/UKDA-5340-tab/tab/household_eul_2002-2020.tab",col_names = TRUE,
...
# Read in individuals
rawData$persons <- read_tsv("data/England/UKDA-5340-tab/tab/individual_eul_2002-2020.tab",col_names = TRUE,
                        cols_only(SurveyYear = "i", HouseholdID = "i", PersNo = "i", WkMode_B01ID = "i",
                                  ...
                                  Age_B01ID = "i",Sex_B01ID = "i", DrivLic_B02ID = "i", TicketHolding_B01ID = "i", OwnCycle_B01ID = "i", CarsEas_B01ID = "i"))

# Read in trips
rawData$trips <- read_tsv("data/England/UKDA-5340-tab/tab/trip_eul_2002-2020.tab",col_names = TRUE,
                      cols_only(SurveyYear = "i", HouseholdID = "i", PersNo = "i",TravDay = "i",JourSeq = "i",HowComp_B01ID = "i",
                                ...
                                TripDisIncSW = "n",JD = "n",TripTotalTime = "n",JJXSC = "i",JTTXSC = "n",W5 = "n"))

# Read in stages (for access & egress)
rawData$stages <- read_tsv("data/England/UKDA-5340-tab/tab/stage_eul_2002-2020.tab", col_names = TRUE,
...
households <- households %>% filter(SurveyYear >= 2014, W1 == 1)

# Filter person data age 10+ only.
persons <- persons %>% semi_join(households["hh.id"])
...
persons <- persons %>% within({
  ...
  # Age group (1 = 0-18, 2 = 19-29, 3 = 30-49, 4 = 50-59, 5 = 60-69, 6 = 70+)
  p.age_gr <- case_when(Age_B01ID <= 7 ~ 1,
                        Age_B01ID <= 12 ~ 2,
                        Age_B01ID <= 14 ~ 3,
                        Age_B01ID <= 15 ~ 4,
                        Age_B01ID <= 17 ~ 5,
...
# Trip Purpose
trips[["t.purpose"]] <- with(trips, case_when((t.origin == "H" & t.destination == "J") ~  "J",
...
trips[["t.full_purpose"]] <- with(trips, case_when((t.origin == "J" | t.destination == "J") ~ "J",
...
# trips[["t.distance"]] <- with(trips, JD * 1.60934)
...
# Travel Time
trips[["t.travelTime"]] <- with(trips,TripTotalTime)
...
NTS <- list()
NTS$rawData <- rawData
NTS$households <- select(households, starts_with("hh."))
NTS$persons <- select(persons, starts_with(c("hh.","p.")))
NTS$trips <- select(trips, starts_with(c("hh.","p.","t.")))
NTS$stages <- select(stages, starts_with(c("hh.","p.","t.","s.")))
