...
threshold <- quantile(trips$dist, 0.99, na.rm = TRUE)
trips_final <- trips %>%  
  filter(
  ...
  mutate(
    occupation = case_when( 
      studying %in% c("Full-time TAFE/Uni", "Part-time TAFE/Uni", "Primary", "Secondary")  ~ "Student", 
      mainact %in% c("Full-time TAFE/Uni", "Part-time TAFE/Uni", "Primary School", "Secondary School", "Other Education") ~ "Student",
      mainact %in% c("Casual Work", "Full-time Work", "Part-time Work")   ~ "Employed (not studying)", 
      mainact %in% c("Keeping House", "Unemployed", "Not Yet at School")  ~ "Unemployed (not studying)", 
...
trips_final <- trips_final %>%
  mutate(
    demographic_status = case_when(
      ...
      age_group != "0-17" & occupation == "Employed (not studying)" & car_ownership == "noCar" ~ "Employed (not studying), no car",
      age_group != "0-17" & occupation == "Employed (not studying)" & car_ownership != "noCar" ~ "Employed (not studying), with car",

      (age_group == "65+" & occupation == "Unemployed (not studying)" & car_ownership == "noCar") | (occupation == "Retired" & car_ownership == "noCar") ~ "Retired, no car",
      (age_group == "65+" & occupation == "Unemployed (not studying)" & car_ownership != "noCar") | (occupation == "Retired" & car_ownership != "noCar") ~ "Retired, with car",

      age_group != "0-17" & age_group != "65+" & occupation == "Unemployed (not studying)" & car_ownership == "noCar" ~ "Unemployed (not studying), no car",
      age_group != "0-17" & age_group != "65+" & occupation == "Unemployed (not studying)" & car_ownership != "noCar" ~ "Unemployed (not studying), with car",
    ))
...
for (cat in categories) {
  format <- gsub("_", " ", cat)
  cat("\n\n### Total number of trips and percentage by ", format, "\n\n")
...
for (cat in categories) {
  ...
  table <- trips_final %>%
    group_by(full_purpose, level = .data[[cat]]) %>%
    summarise(n_trips = n(), .groups = "drop") %>%
    mutate(pct = n_trips / sum(n_trips)) %>% 
    mutate(pct = sprintf("%.1f%%", 100 * pct)) %>% 
    rename(!!format := level,
...
for (cat in categories) {
  ...
  print(
    tbl %>%
      kable("html", align = "l", col.names = c(pretty, "Persons", "Percent (%)")) %>%
      kable_styling(
        full_width = FALSE, 
        font_size = 18,
        bootstrap_options = c("striped","hover","responsive") 
...
This section presents tables summarising the mean, median, 25th percentile, and 75th percentile distance travelled across each category of the selected demographic and socioeconomic categories and trip purposes.
...
max <- quantile(trips$dist, 0.90, na.rm = TRUE) 
trips_plots <- trips_final %>% 
   filter(dist < max ) # filter the distance for better visualisation

for (cat in categories) {
  cat("Trip distance by ", gsub("_", " ", cat))
  
  plot <- ggplot(trips_plots, aes_string(x = cat, y = "dist")) +
