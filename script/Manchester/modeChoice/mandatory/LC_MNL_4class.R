...
trips =  trips%>% within({ 
  ...
  bike_ambienceLogDistance = bike_ambience * bike_logDist
  ...
  walk_lightNightLogDistance = walk_lightNight * walk_logDist
  
  walk_ambienceLogDistance = walk_ambience * walk_logDist
})
...
# sdCor = cor(trips%>%select('hh.cars','hh.bikes','hh.adults','hh.children5to9',
...
beta_lc_matrix <- as.matrix(data.frame(
  ...
  # p.age_group_agg_40_69 = c(1, 0, 0, 0),
  # p.age_group_agg_70    = c(1, 0, 0, 0),
  # p.occupation_student  = c(1, 0, 0, 0),
  # hh.cars_agg            = c(1, 0, 0, 0),
  # hh.cars_gr_3          = c(1, 0, 0, 0),
  # hh.children5to9       = c(1, 0, 0, 0),
  # hh.children10to14     = c(1, 0, 0, 0),
  # walk_dist              = c(1, 0, 0, 0),
  row.names = class
))
...
beta_av_matrix =  as.matrix(data.frame(asc                   = c( 1     , 1     , 1    , 1      ,1),
                                       ...
                                       #shannon               = c( 0     , 0     , 0     , 1     ,1),
                                       #ambienceLogDistance   = c( 0     , 0     , 0     , 1     ,1),
                                       #POIs                  = c( 0     , 0     , 0     , 1     ,1),
                                       #vgviDay               = c( 0     , 0     , 0     , 0     ,1),
                                       #lightNightLogDistance  = c( 0     , 0     , 0     , 1     ,1),
                                       row.names = options))
...
apollo_fixed = c(names(apollo_beta)[c(as.numeric(beta_lc_matrix) == 1,as.numeric(beta_gv_matrix) == 1,unlist(beta_av_class)==FALSE)],
                 "walkav.walk_asc_c1","walkav.walk_asc_c2","walkav.walk_asc_c3","walkav.walk_asc_c4")
...
apollo_lcPars = function(apollo_beta, apollo_inputs){
  ...
  lc = names(apollo_beta)[endsWith(names(apollo_beta),"_c1")|endsWith(names(apollo_beta),"_c2")|
                            endsWith(names(apollo_beta),"_c3")|endsWith(names(apollo_beta),"_c4")]
...
#betaAv=names(apollo_beta)[startsWith(names(apollo_beta),"carDav.")&endsWith(names(apollo_beta),paste0("_",s))]
...
#model=apollo_loadModel("lcmnl_v3")
...
predict = predict$model %>% mutate(
  predictMode = case_when(chosen == carD ~ "carD",
                          chosen == carP ~ "carP",
                          chosen == pt ~ "pt",
                          chosen == walk ~ "walk",
                          TRUE ~ "bike")
