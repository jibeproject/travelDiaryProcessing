...
trips =  trips%>% within({ 
  ...
  p.age_65up = ifelse(p.age_group == "65-69" | p.age_group == "70-74" | 
                  p.age_group == "75-79" | p.age_group == "80-84" | p.age_group == "85+",1,0)
  ...
  #interaction
  carD_timeHBW = case_when(t.full_purpose=="HBW" ~ car_time, TRUE ~ 0)
  carD_timeHBE = case_when(t.full_purpose=="HBE" ~ car_time, TRUE ~ 0)
  carP_timeHBW = case_when(t.full_purpose=="HBW" ~ car_time, TRUE ~ 0)
  carP_timeHBE = case_when(t.full_purpose=="HBE" ~ car_time, TRUE ~ 0)
...
# Trips for Java
tripsForJava = trips %>% 
  filter(t.full_purpose%in%c("HBW","HBE")) %>%
...
write_csv(tripsForJava,"data/Manchester/processed/dynamicLCP/commute.csv")

# Setup for Java (HBW)
tripsForJavaHBW <- tripsForJava %>% filter(t.full_purpose_HBW == 1)

# Setup for Java (HBE)
tripsForJavaHBE <- tripsForJava %>% filter(t.full_purpose_HBE == 1)

with(tripsForJavaHBE,table(av_carD,hh.cars_gr))
...
# Tests
bike_data <- read_csv("../manchester/estimation/StaticRouteData_bike.csv")
walk_data <- read_csv("../manchester/estimation/StaticRouteData_walk.csv")

test <- bike_data %>% transmute(g_bike_grad = g_bike_grad / bike_time,
                                g_bike_stressLink = g_bike_stressLink / bike_time,
                                g_bike_stressLink_f = g_bike_stressLink_f / bike_time)

test <- walk_data %>% transmute(g_walk_vgvi = g_walk_vgvi / walk_time,
                                g_walk_stressJct = g_walk_stressJct / walk_time,
                                g_walk_aadt = g_walk_aadt / walk_time)

ggplot(test, aes(x = g_walk_stressJct, colour = greener)) + geom_density() + xlim(0,0.1)

ggplot(test, aes(x = g_walk_stressJct,g_walk_vgvi)) + geom_point() + xlim(0,0.1)

test %>% cor %>% corrplot(type = "upper", tl.pos = "td", method = "circle", tl.col = 'black', diag = FALSE)


test <- tripsForJavaHBW %>% left_join(bike_data,by = c("t.ID" = "ID")) %>% left_join(walk_data,by = c("t.ID" = "ID"))
males <- test %>% filter(p.female == 0)
females <- test %>% filter(p.female == 1)
with(tripsForJavaHBW,table(t.m_main_apollo,p.age_group_agg))
...
beta_gv_matrix =  as.matrix(data.frame(asc                   = c( 1     , 0     , 0    , 0      ,0),
                                      ...
                                      # p.age_group_agg_25_39 = c( 1     , 0     , 0    , 0      ,0),
                                       p.age_group_agg_55    = c( 1     , 1     , 0    , 0      ,1),
...
beta_av = c(beta_car_time = 0, beta_pt_time = 0, 
            beta_walk_cost = 0, gamma_walk_grad = 1, gamma_walk_vgvi = 1, gamma_walk_stressLink = 1, gamma_walk_stressJct = 0,
            beta_bike_cost = 0, gamma_bike_grad = 0, gamma_bike_vgvi = 1, gamma_bike_stressLink = 0, gamma_bike_stressJct = 1)

beta_av_fixed = c("gamma_walk_grad","gamma_walk_stressLink","gamma_bike_vgvi","gamma_bike_stressJct")
...
model = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs,
                        estimate_settings=list(writeIter=FALSE, estimationRoutine = "bfgs"))
