...
beta_lc_matrix <- as.matrix(data.frame(
  asc                     = c(1, 0, 0),
  p.age_group_agg_15_24   = c(1, 0, 0),
  p.age_group_agg_25_39   = c(1, 0, 0),
  p.female_FALSE          = c(1, 0, 0),
  p.occupation_worker     = c(1, 0, 0),
  hh.cars_gr_0            = c(1, 0, 0),
  hh.cars_gr_1            = c(1, 0, 0),
  hh.cars_gr_2            = c(1, 0, 0), 
  hh.size                 = c(1, 0, 0),
  hh.children5to9_yes     = c(1, 0, 0),
  hh.children10to14_yes   = c(1, 0, 0),
  t.departureTime_day_TRUE= c(1, 0, 0),
  t.full_purpose_HBR      = c(1, 0, 0),
  t.full_purpose_HBS      = c(1, 0, 0),
  t.full_purpose_HBA      = c(1, 0, 0),
  t.full_purpose_HBO      = c(1, 0, 0),
  # hh.cars_agg             = c(1, 0, 0),
  # Uncomment the lines below and replace 0 with the desired values
  # p.age_group_agg_5_14    = c(1, 0, 0),
  # p.age_group_agg_40_54   = c(1, 0, 0),
  # p.age_group_agg_55    = c(1, 0, 0),
  # p.occupation_student  = c(1, 0, 0),
  # hh.cars_gr_3          = c(1, 0, 0),
  # hh.carsPerAdult       = c(1, 0, 0),
  #hh.cars_gr_3            = c(1, 0, 0), 
  #hh.children             = c(1, 0, 0),
  # hh.children_yes       = c(1, 0, 0),
  row.names = class
))
...
apollo_beta = c(rep(0,length(beta_lc)),rep(0,length(beta_gv)),rep(0,5),rep(-0.1,3),rep(0,10),rep(0,5),rep(-0.1,3),rep(0,10),rep(0,5),rep(-0.1,3),rep(0,10))
names(apollo_beta) = c(beta_lc,beta_gv,beta_av_class)

apollo_beta = apollo_beta[names(apollo_beta) %in% c("ptav.pt_asc_c1","ptav.pt_time_c1",
                                                    "bikeav.bike_asc_c1","bikeav.bike_logDistkm_c1","bikeav.bike_stressLink_c1",
                                                    "bikeav.bike_stressJctDistance_c1","bikeav.bike_crimeDistance_c1","bikeav.bike_vgviDay_c1",
                                                    "carDav.carD_asc_c2","carDav.carD_time_c2",
                                                    "ptav.pt_asc_c3","ptav.pt_time_c3",
                                                    "bikeav.bike_asc_c3","bikeav.bike_logDistkm_c3","bikeav.bike_stressLink_c3",
                                                    "bikeav.bike_stressJctDistance_c3","bikeav.bike_crimeDistance_c3","bikeav.bike_vgviDay_c3") == FALSE]
...
apollo_lcPars = function(apollo_beta, apollo_inputs){
  ...
  lc = names(apollo_beta)[endsWith(names(apollo_beta),"_c1")|endsWith(names(apollo_beta),"_c2")|endsWith(names(apollo_beta),"_c3")]
  ...
  for (var in vars) {
    ...
    lcpars[[var]] = list(var1,var2,var3)
  ...
  classAlloc_settings = list(
    classes      = c(Class_c1=1, Class_c2=2, Class_c3=3), 
    utilities    = V
  )
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate_settings$bootstrapSE=TRUE"){
  ...
  availa = list()
  availa[["c1"]] = list(carD=av_carD, carP=1,pt=1, bike=1,walk=1)
  availa[["c2"]] = list(carD=0, carP=1,pt=1, bike=1,walk=1)
  availa[["c3"]] = list(carD=av_carD, carP=1,pt=1, bike=1,walk=1)
...
model = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs,
                        estimate_settings=list(writeIter=FALSE,maxIterations=500,
                                               constraints = c("carDav.carD_time_c1 < 0",
                                                               "carPav.carP_time_c1 < 0",
                                                               "carDav.carD_time_c3 < 0",
                                                               "carPav.carP_time_c3 < 0")))
...
model=apollo_loadModel("lcmnl_v14")
apollo_modelOutput(model)
database$walk_vgviDay=0
apollo_inputs = apollo_validateInputs()
predictMode = apollo_prediction(
  model,
...
predictMode = predictMode$model %>% mutate(
  ...
  predictMode = case_when(maxProb == carD ~ "carD",
                          ...
                          maxProb == walk ~ "walk",
                          TRUE ~ "bike"),
  chosentMode = case_when(chosen == carD ~ "carD",
                          chosen == carP ~ "carP",
                          chosen == pt ~ "pt",
                          chosen == walk ~ "walk",
                          TRUE ~ "bike")
...
sum(predictMode$walk)

database$predict = predictMode$predictMode
...
indicators=trips[,c("t.ID","p.age_group_agg_5_14","p.age_group_agg_15_24",
                    ...
                    "t.departureTime_day_TRUE","t.departureTime_day_FALSE",
                    "t.full_purpose_HBR","t.full_purpose_HBS","t.full_purpose_HBA","t.full_purpose_HBO")]
expectedValue = expectedValue%>%
  gather(class,weight,2:4)%>%
...
summary=summary%>%gather(indicator, ev, 2:21)%>%spread(class,ev)
...
beta_av_full = apply(expand.grid(options,colnames(beta_av_matrix))%>%mutate(Var1 = paste(Var1,Var1,sep = "av.")), 1, paste, collapse="_")
estimateValues_full <- c(rep("0",length(beta_lc)),rep("0",length(beta_gv)),rep("0",length(beta_av_full)))
names(estimateValues_full) = c(beta_lc,beta_gv,beta_av_full)
L <- list(as.list(estimateValues_full),as.list(estimateValues))
estimateValues_full =data.table::rbindlist(L, fill=TRUE)[2]

estimate_coeffsMatrix <- matrix(estimateValues_full,nrow = ncol(beta_lc_matrix)+length(beta_gv)+length(beta_av), ncol = length(class), byrow = TRUE, dimnames = list(c(colnames(beta_lc_matrix),colnames(beta_gv),colnames(beta_av)),options))
