...
trips = trips%>%filter(t.full_purpose%in%c("HBR","HBS","HBO"))
...
beta_av = c(beta_car_time = -0.01, beta_pt_time = -0.01, 
            beta_walk_cost = -0.01, gamma_walk_grad = 0, gamma_walk_vgvi = 0.01, gamma_walk_stressLink = 0, gamma_walk_stressJct = 0,
            beta_bike_cost = -0.01, gamma_bike_grad = 0, gamma_bike_vgvi = 0, gamma_bike_stressLink = 0, gamma_bike_stressJct = 0)
...
apollo_beta = c(rep(0,length(beta_gv)),beta_av)
...
apollo_fixed = c(beta_gv[as.numeric(beta_gv_matrix) == 1],beta_av_fixed)
...
interzonalBike <- read_csv("data/manchester/routed/multiRoutesBike7.csv", col_names = TRUE, 
  ...
  group_by(t.ID) %>% slice(which.min(travelTime)) %>%
  semi_join(database)

interzonalWalk <- read_csv("data/manchester/routed/multiRoutesWalk5.csv", col_names = TRUE, 
                           col_types = cols_only(IDNumber = "c", PersonNumber = "i", TripNumber = "i",
                                                 travelTime = "n", gradient = "n", vgvi = "n", stressLink = "n", stressJct = "n")) %>%
  mutate(t.ID = paste0(IDNumber,PersonNumber,TripNumber)) %>%
  relocate(t.ID) %>% select(-IDNumber,-PersonNumber,-TripNumber) %>%
...
routes <- list()
routes$bike <- rbind(interzonalBike,intrazonalBike) %>% rename_at(vars(-t.ID),~paste0(.,"_bike"))
routes$walk <- rbind(interzonalWalk,intrazonalWalk) %>% rename_at(vars(-t.ID),~paste0(.,"_walk"))
...
database <- database %>% inner_join(routes$bike) %>% inner_join(routes$walk)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality){
  ...
  V[['bike']] = calcV(names(apollo_beta)[startsWith(names(apollo_beta),"bike.")]) + 
    beta_bike_cost * (travelTime_bike + 
                        gamma_bike_grad * gradient_bike + 
                        gamma_bike_vgvi * vgvi_bike + 
                        gamma_bike_stressLink * stressLink_bike + 
                        gamma_bike_stressJct * stressJct_bike)
  V[['walk']] = calcV(names(apollo_beta)[startsWith(names(apollo_beta),"walk.")]) + 
    beta_walk_cost * (travelTime_walk + 
                        gamma_walk_grad * gradient_walk + 
                        gamma_walk_vgvi * vgvi_walk + 
                        gamma_walk_stressLink * stressLink_walk + 
                        gamma_walk_stressJct * stressJct_walk)
