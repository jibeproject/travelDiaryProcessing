# This script estimates the trip attractions for the destination choice model by considering power transforms to the POI weights.
...
models$shop <- lm(shop ~
                    ...
                    #EE +
                    #PHC +
                    SER, data= dataset %>% filter(shop >0))
...
summary(models$escort) 
# TODO: remove trip attractions to airport / %>% filter(IDspatial != "E01005316")
...
#}else{
#  modelsummary(models, output = "manchesterData/tripAttractionsCoefficients_withStats_without_PowerTransforms.png")
#}
modelsummary(models, output = "manchesterData/tripAttractionsWITH_transformations_Corin_et_al_2025.png")


modelsummary(models, output = "manchesterData/tripAttractionsCoefficients_withPT.png")
...
modelSummaryAtttractions<-modelSummaryAtttractions %>%
  ...
  dplyr::select(Variable, work, education, shop, escort, recreation, other, rrt)
...
A <- modelSummaryAtttractions %>%
  dplyr::select(work, education, shop, escort, other, recreation, rrt) %>%
  mutate(across(everything(), as.numeric)) %>%
  replace(is.na(.), 0)
B <- table %>% dplyr::select(work, education, shop, escort, other, recreation, rrt)
A<- as.matrix(A)
B <- t(as.matrix(B))
C<-A%*%B
df <- data.frame(C)
...
df$poi <- rowNames
df <- df %>% dplyr::select(poi, HBW, HBE, HBA, HBO, HBR, HBS, NHBO, NHBW, RRT)
# adjust SCL as it is rejected from all models
# Create a new row with zeros and poi as "SCL"
new_row <- data.frame(
  poi = "RSPF",
  HBW = 0, HBE = 0, HBA = 0, HBO = 0, HBR = 0, HBS = 0, NHBO = 0, NHBW = 0, RRT=0
)
